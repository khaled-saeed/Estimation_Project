...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  // make sure you comment it out when you add your own code -- otherwise e.g. you might integrate yaw twice
    ...
    Mat3x3F Rotatin = Mat3x3F();
    Rotatin(0, 0) = 1;
    Rotatin(0, 1) = sin(phi) * tan(theta);
    Rotatin(0, 2) = cos(phi) * tan(theta);
    Rotatin(1, 0) = 0;
    Rotatin(1, 1) = cos(phi);
    Rotatin(1, 2) = -sin(phi);
    Rotatin(2, 0) = 0;
    Rotatin(2, 1) = sin(phi) / cos(theta);
    Rotatin(2, 2) = cos(phi) / cos(theta);

    V3F euler_dot = Rotatin * gyro; // gyro [p ,q ,r ]
...
VectorXf QuadEstimatorEKF::PredictState(VectorXf curState, float dt, V3F accel, V3F gyro)
{
  ...
  //   accel: acceleration of the vehicle, in body frame, *not including gravity* [m/s2] ?X_dot , Y_dot , Z_dot
  //   //   gyro: body rates of the vehicle, in body frame [rad/s] theta_dot , alpha_dot , psi_dot 
  //   
  // OUTPUT:
...
void QuadEstimatorEKF::Predict(float dt, V3F accel, V3F gyro)
{
  ...
  gPrime(1, 4) = dt;
  gPrime(2, 5) = dt;
  gPrime(3, 6) = (RbgPrime(0) * accel).sum() * dt; // dot product between RbgPrime(0) & accel
...
void QuadEstimatorEKF::UpdateFromMag(float magYaw)
{
  ...
  zFromX(0) = ekfState(6);  // the 6th element in the state vector [x ,y ,z , x_dot , y_dot , z_dot, psi] 
  float diff = magYaw - zFromX(0); // the difference between the measured and estimated yaw
